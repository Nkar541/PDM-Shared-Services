# -*- coding: utf-8 -*-
"""
Created on Wed Mar  9 22:24:08 2022
...
import pyodbc
import numpy as np
filename = r"C:\Users\NKar\Music\Meritus_Adds_Nk.xlsx"


df = pd.read_excel(filename,sheet_name = 'Sheet1')


print('Kindly perform  data cleansing manually in order to match data values to data columns')
print('Use the cleansed csv file in the path')

# columns that dont change
df.rename(columns={'Indiv. NPI#':'prov_npi_id', 'Last Name': 'prov_last_name', 'First Name': 'prov_first_name'}, inplace=True)
df.rename(columns={'M I ':'prov_middle_initial', 'Degree': 'prov_prof_desg_1', 'Gender': 'prov_gender'}, inplace=True)
df.rename(columns={'Ethnicity': 'prov_ethnicity','Languages':'prov_language_1'}, inplace=True)
df.rename(columns={'Birth Date':'prov_birth_date'}, inplace=True)
df.rename(columns={'Federal Tax ID':'tax_id_number','GROUP NPI #':'bu_npi_id','OFFICE NAME':'bu_name'}, inplace=True)
df.rename(columns={'Mailing address (Billing)':'bu_address_1'}, inplace=True)
df.rename(columns={'bill_city':'bu_city','bill_state':'bu_state','bill_zip':'bu_zip_code'}, inplace=True)

df.rename(columns={'Address #1':'sl_address_1','Suite/Unit #':'sl_address_2','City ':'sl_city','State ':'sl_state'}, inplace=True)
df.rename(columns={'Zip': 'sl_zip_code','Office County':'sl_county', 'Address #1':'sl_mail_address_1'}, inplace=True)
df.rename(columns={'Suite/Unit #':'sl_mail_address_2','City':'sl_mail_city','State ':'sl_mail_state'}, inplace=True)
df.rename(columns={'Zip':'sl_mail_zip_code','PRI Phone number':'sl_phone_number','PRI FAX':'sl_fax_number'}, inplace=True)
df.rename(columns={'PCP/Specialist':'sl_status_code',})

df['AgeLimits']=df['AgeLimits'].replace(np.nan,'0-100',regex=True)

df[['Min_Age','Max_Age']] = df['AgeLimits'].str.split('-',expand=True)

#add in blank columns
df['prov_gender']=df['prov_gender'].replace(["Female","Male","female","male"],["F","M","F","M"])
df['prov_name_suffix']=''
df['prov_soc_sec_number']=df['SOC SEC #']
df['prov_language_2']=''
df['prov_email_address']=''
df['prov_medicare_id']=''
df['prov_medicaid_id']=df['Medicaid #']
...
df['sl_status_code']=df['PCP/Specialist'].replace(["Specialist","PCP"],["N","Y"])
df['sl_accept_new_patient']=df['Accepting New Patients'].replace(["No","Yes"],["N","Y"])
df['sl_min_patient_age']=df['Min_Age']
df['sl_max_patient_age']=df['Max_Age']
df['sl_offered_services']=''
df['sl_show_in_dir']=df['List in Directory'].replace(["No","Yes"],["N","Y"])
df['sl_tty_service']=''
...
df['sl_accepts_physical_disabilities']='Y'
...
df['sl_service_location_name']=''
df['sl_termination_date']=''
df['spec_specialty_id_1']=df['Taxonomy Code']
...
df['cert_cert_name_3']=''
df['aff_effective_date']=df['Delegate Committee Approval Date']
...
df['lic_state_1_id']=df['Medical Lic #']
df['lic_state_1_state']='MD'
df['lic_state_1_eff_date']='1/1/2020'
df['lic_state_1_term_date']=df['Expiration Date']
df['lic_state_2_id']=''
...
df['lic_dea_1_id']=df['DEA #']
df['lic_dea_1_state']='MD'
df['lic_dea_1_eff_date']='1/1/2020'
df['lic_dea_1_term_date']=df['DEA Exp']
df['lic_dea_2_id']=''
df['lic_dea_2_state']=''
...
df = df[['prov_npi_id','prov_last_name','prov_first_name','prov_middle_initial','prov_name_suffix','prov_prof_desg_1','prov_soc_sec_number',
           'prov_gender','prov_language_1','prov_language_2','prov_ethnicity','prov_birth_date','prov_email_address','prov_medicare_id',
...
df.to_csv('Meritus_Adds.csv',index=False)
...
df_mapped["concat"] = df_mapped['prov_npi_id'].astype(str)+"_"+df_mapped['tax_id_number'].astype(str)+"_"+df_mapped['bu_npi_id'].astype(str)+ "_"+ df_mapped['sl_address_1'].str[:4].str.lower() + "_" + df_mapped['bu_address_1'].str[-5:]

aldera['concat']= aldera['prov_npi_id'].astype(str)+"_"+aldera['tax_id_number'].astype(str)+"_"+aldera['bu_npi_id'].astype(str)+ "_"+ aldera['sl_address_1'].str[:4].str.lower() + "_" + aldera['bu_address_1'].str[-5:]
...
df_mapped_aldera.to_csv('Mapped_Meritus.csv',index=False)
...
#df_language.to_csv('Mapped_HC3.csv',index=False)
...
alderaJoin.to_csv('Meritus_mapped.csv',index=False)
