# -*- coding: utf-8 -*-
"""
Created on Tue May 16 22:11:09 2023
...
print('Running Conversion Query for CCHS Roster')
...
#df = df.replace('nan', '')
#df =df.fillna('')
df['Taxonomy Code'] = df['Taxonomy Code'].apply(lambda x: x.replace('&',','))

df['Taxonomy Code']=df['Taxonomy Code'].str.split('[,]')
df = df.explode('Taxonomy Code').reset_index(drop=True)
...
df['Delegated Group Entity Name']='ChristianaCare Health Services'
df['Education Type']=''
df['Start Date_Inst']=''
df['Speciality Type']=''
...
df[['Start Date-Residency', 'End Date-Residency']] = ''
...
boardfilename = r'M:\Provider Data\MPMD\Spayer\Board Name Crosswalk_Final.xlsx'
...
institutionfilename = r'M:\Provider Data\MPMD\Spayer\Institution Crosswalk_FINAL.xlsx'
...
specialtyfilename = r'M:\Provider Data\MPMD\Spayer\Taxonomy Codes_2021.07.15.xlsx'
...
degreefilename = r'M:\Provider Data\MPMD\Spayer\Degree Crosswalk_Final.xlsx'
...
hospfilename = r'M:\Provider Data\MPMD\Spayer\Institution Crosswalk_FINAL.xlsx'
...
df_mapped_final.to_excel('CCHS_' +yyyymmdd+ '_spayer.xlsx',index=False)
df_mapped_final.to_csv('CCHS_' +yyyymmdd+ '_spayer.csv',index=False)
df_mapped_final.to_csv('CCHS_' +yyyymmdd+ '_spayer.txt',index=None, sep='\t')
