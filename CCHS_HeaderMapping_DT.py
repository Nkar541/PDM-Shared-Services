# -*- coding: utf-8 -*-
"""
Created on Fri Jan  5 15:59:05 2024
...
filename = r'C:\Users\AANair\Downloads\Christiana Care inc recd 05172024.xlsx'
...
df.rename(columns={'Birth Date':'prov_birth_date','SOC SEC #': 'prov_soc_sec_number'}, inplace=True)
df.rename(columns={'Federal Tax ID':'tax_id_number','GROUP NPI #':'bu_npi_id'}, inplace=True)
...
df.rename(columns={'Zip':'sl_zip_code','PRI Phone number':'sl_phone_number','PRI FAX':'sl_fax_number'}, inplace=True)
df.rename(columns={'PCP/Specialist':'sl_status_code','Taxonomy Code':'spec_specialty_id_1'}, inplace=True)
df.rename(columns={'List in Directory':'sl_show_in_dir','Accepting New Patients':'sl_accept_new_patient'}, inplace=True)
df.rename(columns={'Medicaid #':'prov_medicaid_id','Medicare #':'prov_medicare_id'}, inplace=True)
df.rename(columns={'DEA #':'lic_dea_1_id','DEA Exp':'lic_dea_1_term_date'}, inplace=True)
...
df['sl_min_patient_age']=df[' Age Limits']
df['sl_max_patient_age']=df[' Age Limits']
...
df['sl_service_location_name']=df['OFFICE NAME']
...
alderaJoin.to_csv('cchs_mapped_final.csv',index=False)
